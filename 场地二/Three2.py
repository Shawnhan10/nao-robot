...
g_moveConfig = [["MaxStepX", 0.04], ["MaxStepY", 0.13], ["MaxStepTheta", 0.4], ["MaxStepFrequency", 0.55],
                ["StepHeight", 0.024], ["TorsoWx", 0.0], ["TorsoWy", 0.0]]
# LandMark及ball参数
landmark = {"size": 0.09}
...
#             g_motion.angleInterpolationWithSpeed("LWristYaw", -0.5, 0.17)#-0.40.15,
...
def firstHitBallForThree():
    ...
    while True:
        if g_memory.getData("HandRightRightTouched"):
            g_motion.angleInterpolationWithSpeed("LWristYaw", -0.3, 0.08)  # 本地0.2
            g_motion.angleInterpolationWithSpeed("LWristYaw", 0.4, 0.05)
            break
...
def move(x=0.0, y=0.0, theta=0.0, config=g_moveConfig):
    ...
    try:
        ...
        x2 = round(x)
        y1 = round(y)
        theta1 = round((theta + 2.5) * almath.TO_RAD, 2)
        # 行走前初始化
        g_motion.moveInit()
        # 如果是往前走，修正
        step1 = x1 / 56
...
def move2(x=0.0, y=0.0, theta=0.0, config=g_moveConfig1):
    ...
    try:
        ...
        x2 = round(x)
        y1 = round(y)
        theta1 = round((theta+3) * almath.TO_RAD, 2)
        # 行走前初始化
        g_motion.moveInit()
        # 如果是往前走，修正
        step1 = x1 / 56
...
def secondHitBallForThree():
    ...
    g_motion.angleInterpolationWithSpeed("LElbowYaw", -1.8, 0.2)
    g_motion.angleInterpolationWithSpeed("LElbowRoll", -0.83, 0.2)
    g_motion.angleInterpolationWithSpeed("LWristYaw", 0.72, 0.05)
    g_motion.angleInterpolationWithSpeed("LShoulderRoll", -1.5, 0.2)
...
def secondHitBallForThree1():
    ...
    # 出杆
    # // 第二次击球的准备动作（超远击球）
    yawDegreex = 90 * almath.TO_RAD
    g_motion.angleInterpolationWithSpeed("HeadYaw", -yawDegreex, 0.4)
    time.sleep(1)
    ...
    # // 击球（超远击球）
    g_motion.angleInterpolationWithSpeed("LWristYaw", 0.6, 0.1)  # 1.2
    g_motion.angleInterpolationWithSpeed("LWristYaw", -0.55, 0.21)  # 0.6 0.18
    # g_motion.angleInterpolationWithSpeed("LWristYaw", -0.6, 0.17)
    g_motion.angleInterpolationWithSpeed("LWristYaw", 0.4, 0.05)
                ...
                if adjustY < 24 and adjustY > -24:
                    ...
                    if adjustTheta > 0:
                      ...
                      # 右边
                        ...
                        #     move(theta=adjustTheta + 4)  # ..√
                        # elif -30 >= adjustTheta > -40:
                        #     move(theta=adjustTheta + 6)  # ..√
                        # elif -40 >= adjustTheta > -50:
                        #     move(theta=adjustTheta + 9)  #
                        # else:
                        #     move(theta=adjustTheta + 10)
                elif adjustY > 0:
                    if adjustTheta > 0:
                        move(theta=15)
                    else:
...
def thirdMain():
    ...
    move(y=-1)
    #trackBall(headang)
    # move(y=-3)  # -3#-4#解决往左转8度，球y坐标会减少3的问题
    # move(theta=-2)  # 5#8#往左转8度，与球的距离增大2，球y坐标减少3
    # move(x=2)  # 2#1
