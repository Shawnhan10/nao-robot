...
def trackBallfortwo(headPitch=0):
    ...
    while True:
        ...
        # 没找到球
            ...
            # 判断扭头次数，扫描3遍
            if times % 3 == 0:
                g_motion.angleInterpolationWithSpeed("HeadYaw", 0, 0.3)
                times += 1
                time.sleep(0.5)
            elif times % 3 == 2:
                g_motion.angleInterpolationWithSpeed("HeadYaw", -yawDegree, 0.3)
                times += 1
                time.sleep(0.5)
            else:
                g_motion.angleInterpolationWithSpeed("HeadYaw", yawDegree, 0.3)
...
def firstHitBallForTwo():
    ...
    while True:
        if g_memory.getData("HandRightRightTouched"):
            # 红色机器人击球---4.5米完成（场地二）
            g_motion.angleInterpolationWithSpeed("LWristYaw", 0.9, 0.2)  # 0.99 0.26
...
def outShotBall(distance):
    ...
    g_motion.angleInterpolationWithSpeed("LWristYaw", 0.8, 0.2)  # 0.085#>0往外
    g_motion.angleInterpolationWithSpeed("LWristYaw", 0.4, 0.05)
...
def secondMain2():
    ...
    if distance <= 0:  # 如果距离太近找不到用下摄像头，下摄像头没找到后退15，再用上摄像头 PS:如果距离太远找不到重开
